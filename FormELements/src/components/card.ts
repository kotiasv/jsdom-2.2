...
// Filter functions
export const cardInfoFilter = (event: Event) => {
    const value = (event.target as HTMLInputElement).value
    cardInfo.textContent = value.length > 0 ? value : "0000 0000 0000 0000"
}

export const ownerInfoFilter = (event: Event) => {
    const value = (event.target as HTMLInputElement).value
    ownerInfo.textContent = value.length > 0 ? value : "Нет информации"
}

export const dateInfoFilter = (event: Event) => {
    const value = (event.target as HTMLInputElement).value
    dateInfo.textContent = value.length > 0 ? value : "01/01"
}

export const typeInfoFilter = (event: Event) => {
    const value = (event.target as HTMLSelectElement).value

    if (value) {
        if (typeInfo.textContent) typeInfo.textContent = ""
...
export const bankInfoFilter = (event: Event) => {
    const value = (event.target as HTMLSelectElement).value
    ...
    if (value) {
        ...
        if (bankInfo.textContent) bankInfo.textContent = ""
        bankInfo.style.backgroundImage = `url(./public/${banks[
            value.toLowerCase() as "сбербанк" | "тинькофф"
        ]})`
        return
    }
